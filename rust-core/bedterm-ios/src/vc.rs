...
use objc2_ui_kit::{
    UIBarButtonItem, UIBarButtonItemStyle, UIButton, UIColor, UINavigationItem,
...
pub struct Ivars {
    ...
    /// Opaque context pointer threaded back to the C `on_back` callback.
    /// Lifetime is owned entirely by the Swift host: the host registers
    /// the `(callback, ctx)` pair in `bt_ios_create_vc` and is contractually
    /// responsible for keeping `ctx` valid until either it clears the
    /// pair or `bt_ios_release_vc` is called. We never deref it from
    /// Rust — it is only ever passed back through `on_back(ctx)`.
    ///
    /// SAFETY: never dereferenced on the Rust side. The pointee's
    /// validity is the Swift host's responsibility; main-thread access
    /// is enforced because `back_button_tapped` fires from a UIKit
    /// target/action.
    ...
    impl RsTerminalViewController {
        ...
        fn view_did_load(&self) {
            ...
            if let Some(view) = self.view() {
                ...
                // a11y identifier for the VC's root container — UI tests
                // address the whole Rust subtree off this one ID.
            ...
            // Built via the shared factory so the block-list composer
            // (W7) and the in-VC composer use the same configuration.
            let composer = crate::composer_text_view::make_composer_text_view(mtm, self.as_ref());
            let view2 = composer.text_view.clone();
            crate::a11y::set_a11y_id(
                &*view2 as &AnyObject,
                "terminal.rust.composer",
            );

            self.ivars().one_line_height.set(composer.one_line_height);
            self.ivars().three_line_height.set(composer.three_line_height);
            self.ivars().view2_height.set(composer.one_line_height);
            ...
            *self.ivars().view1_tap.borrow_mut() = tap_opt;
            ...
            crate::a11y::set_a11y_id(&*send_chip as &AnyObject, "keybar.run");
            ...
            crate::a11y::set_a11y_id(&*composer_chip as &AnyObject, "keybar.composer");
            ...
            crate::a11y::set_a11y_id(&*close_chip as &AnyObject, "keybar.close");
            ...
            crate::a11y::set_a11y_id(&*hud as &AnyObject, "terminal.rust.hud");
            ...
            *self.ivars().mode_state.borrow_mut() = Some(mode_state);
            ...
            // Seed the HUD's geom/mode chips so they aren't blank pills
            // until the first relayout / mode flip.
            self.refresh_hud_geom_and_mode();
        }

        /// Raw pointer to the embedded `BtIosMetalInputView` (a.k.a. `view1`).
        /// Used by the session-attach FFI to install the external term.
        /// Returns null when the view didn't construct (headless tests).
        #[unsafe(method(btIosMetalView))]
        fn bt_ios_metal_view(&self) -> *const crate::metal_view::BtIosMetalInputView {
            self.ivars()
                .view1
                .borrow()
                .as_ref()
        ...
        /// Raw pointer to the embedded `BtIosKeyboardCoordinator`.
        #[unsafe(method(btIosCoordinator))]
        fn bt_ios_coordinator(&self) -> *const AnyObject {
            self.ivars()
                .coordinator
                .borrow()
                .as_ref()
                .map(|c| &**c as *const _ as *const AnyObject)
                .unwrap_or(std::ptr::null())
        }
        ...
        fn view_did_layout_subviews(&self) {
            ...
            // Refresh HUD geom + mode chips on every layout pass. The
            // metal view's grid_dim is updated from its own
            // `layoutSubviews` → `on_resize`; by the time UIKit calls
            // viewDidLayoutSubviews on the VC, subview layout has
            // already cascaded, so grid_dim is current. Cheap enough to
            // run unconditionally (two setText: calls).
            self.refresh_hud_geom_and_mode();
        }
...
impl RsTerminalViewController {
    /// Push the current PTY grid + InputMode to the debug HUD's geom and
    /// mode chips. Cheap (two `setText:` calls); safe to call from any
    /// layout / mode-change path. Main-thread only.
    fn refresh_hud_geom_and_mode(&self) {
        let hud_borrow = self.ivars().debug_hud.borrow();
        let Some(hud) = hud_borrow.as_ref() else {
            return;
        };
        // Grid: read straight from the metal view's cached grid_dim
        // (kept in sync from its own layoutSubviews → on_resize tick).
        if let Some(ref v1) = *self.ivars().view1.borrow() {
            let (cols, rows) = v1.grid_dim();
            hud.refresh_geom(cols, rows);
        }
        // Mode: short token mirroring the Swift HUD's "inline / block /
        // alt" semantics. State1 = inline input bar, State2 = idle
        // (view1 focusable), State3 = composer.
        let mode_token = match self
            .ivars()
            .mode_state
            .borrow()
            .as_ref()
            ...
            .unwrap_or(InputMode::State2)
        {
            InputMode::State1 => "input",
            InputMode::State2 => "idle",
            InputMode::State3 => "composer",
        };
        hud.refresh_mode(mode_token);
    }
