...
struct ByteSequenceTests {
    ...
    @Test("bare keys from idle emit correct VT100 sequences",
          arguments: [
    ...
    @Test("bare keys from ctrlPending emit the bare sequence and exit pending (MVP: no Ctrl+arrow)",
          arguments: [
    ...
    func bareKeysFromPendingCancelAndSend(_ tap: KeyTap) {
        ...
        if case .bytes = outputs.first { /* ok */ } else {
