...
| T15 | Direction Pad (R11): user-invoked overlay D-pad. `KeyBar` gains a trailing `dpad`-symbol toggle button (next to the R15 keyboard-dismiss key, inside the same Liquid Glass capsule); the toggle drives a `dpadOpen: Bool` `@State` on `TerminalScreen`. When open, a new `DirectionPad` SwiftUI view is placed inside the `ZStack` that hosts `TerminalHostView`, anchored just above the bottom bar via `.overlay(alignment: .bottom)` — it overlays the terminal output without pushing it and without resizing the PTY. Plus-shaped layout: four chevron wedges (`chevron.up/down/left/right`) around a smaller central disc with `xmark`. Each wedge calls `keyBar.handle(.up/.down/.left/.right)` (reuses `KeyBarState.emitArrow` → `ESC [ A\|B\|C\|D`); centre tap sets `dpadOpen = false`. A transparent dismiss layer (`Color.clear.contentShape(Rectangle())`) sits behind the pad inside the terminal `ZStack` and also sets `dpadOpen = false` on tap, so anywhere the user taps over the terminal viewport closes the pad. The bottom bar is **outside** that ZStack (it lives in the parent `VStack`), so taps on the keybar (esc/ctrl/tab, keyboard-dismiss toggle, dpad toggle, compose pill) are never intercepted by the scrim and continue to work normally. Light haptic per direction, medium haptic on centre dismiss. Liquid Glass material (`.glassEffect(.regular.interactive(), in: .circle)`) and `Color.accentLabel` foregrounds match the toolbar. `.onChange(of: composer.isOpen)` auto-sets `dpadOpen = false` when the composer opens (does not reopen on composer close — the user must re-toggle). Sized ~200×200pt; each wedge a ~64pt hit target. Accessibility ids `keybar.dpadtoggle`, `dpad.up`, `dpad.down`, `dpad.left`, `dpad.right`, `dpad.close`. Press-and-hold repeat is **deferred** (PRD R11.hold_repeat). | ❌ |
